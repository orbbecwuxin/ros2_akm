...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::msg::Twist::SharedPtr twist_aux)
{
  ...
  // printf("cmd_vel linear.x: %.3f \n", twist_aux->linear.x);
  // printf("read_vel_x: %.3f \n", Robot_Vel.X );
  // RCLCPP_INFO(this->get_logger(),"cmd_vel linear.x: %.3f ", twist_aux->linear.x);
  // RCLCPP_INFO(this->get_logger(),"read_vel_x: %.3f ", Robot_Vel.X );
  // RCLCPP_INFO(this->get_logger(),"cmd_vel angle.z: %.3f",  twist_aux->angular.z); 
  // RCLCPP_INFO(this->get_logger(),"read_vel_z: %.3f",  Robot_Vel.Z);
    ...
    if(Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
      ...
      if(error == 0)
      {
        ...
        */
        ...
        Mpu6050.angular_velocity.x =  Mpu6050_Data.gyros_x_data * GYROSCOPE_RATIO;
        Mpu6050.angular_velocity.y =  Mpu6050_Data.gyros_y_data * GYROSCOPE_RATIO;
        Mpu6050.angular_velocity.z  = 0.0 * Mpu6050.angular_velocity.z + 1 *( Mpu6050_Data.gyros_z_data * GYROSCOPE_RATIO +0.0163); //+ 0.0163
